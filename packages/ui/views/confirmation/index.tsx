...
class ConfirmationScreen extends Component<Props, State> {
  ...
  displayMessage() {
    ...
    if (language === 'ja' && type === 'erc20Sent') {
      return <Text style={style.text}>
        <Text style={style.nickname}>{type !== 'erc20Sent' ? `@${friend.nickname}` : amount}</Text>
        <Text>{confirmation[type].start}</Text>
        {type === 'erc20Sent' ? <Text> {tokenName}</Text> : null}
        <Text>{confirmation[type].end}</Text>
        {type === 'erc20Sent' ? <Text style={style.nickname}>{txHash}</Text> : null}
      </Text>
    }
